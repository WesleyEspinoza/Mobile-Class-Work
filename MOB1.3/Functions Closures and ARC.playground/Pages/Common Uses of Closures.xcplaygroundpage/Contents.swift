...
let guestToSortSorted = guestsToSort.sorted { (n1, n2) in
    if n1.name <= n2.name {
        return true
    } else{
        return false
...
let guestToSortSortedUnder18 = guestsToSort.filter { n1 in
    if n1.age < 18 {
        return true
    } else{
        return false
...
let nuberstoFilterEven = numbersToFilter.filter { n1 in
    if n1 % 2 == 0 {
        return true
    } else{
        return false
...
let newNew = numbersToDouble.map { (n: Int) -> Int  in
    let newNew = n*2
    return newNew
    }
print(newNew)
...
let numbersIntoStringsDone = numbersToMapIntoStrings.map { (num: Int) -> String in
    let newNew = "\(num)"
    return newNew
}
print(numbersIntoStringsDone)
...
var numbersToSum = [-2, -5, -4, 5, -5, 5]

numbersToSum = numbersToSum.filter { n in
    if n < 0 {
        return false
    } else{
        return true
...
let numbersToSumDone = numbersToSum.reduce(0) { (sum, n) -> Int in
    let newNew = sum + n
    return newNew
}

print(numbersToSumDone)
